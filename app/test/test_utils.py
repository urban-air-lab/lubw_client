...
def test_timestamp_with_offset_format():
    actual = get_timestamps_with_offset()
    datetime.strptime(actual[0], "%Y-%m-%dT%H:%M:%S").replace(
        tzinfo=ZoneInfo("Europe/Berlin")
    )
    datetime.strptime(actual[1], "%Y-%m-%dT%H:%M:%S").replace(
        tzinfo=ZoneInfo("Europe/Berlin")
    )
...
def test_UTF8BasicAuth():
    response = requests.Request(
        "GET", url="https://www.google.de", auth=UTF8BasicAuth("user", "password")
    )
...
def test_fetch_station_data(response_mock):
    ...
    expected = pd.DataFrame(
        {"datetime": pd.Timestamp("2025-05-18T18:00:00+01:00"), "PM10": 14.14},
        index=[0],
    )
...
def test_get_lubw_data_success(response_mock):
    ...
    parameters = {
        "komponente": "PM10",
        "von": "2025-05-18T17:00:00",
        "bis": "2025-05-18T18:00:00",
        "station": "DEBW015",
    }

    result = get_lubw_data(
        session=requests.Session(), params=parameters, next_link=None
    )
